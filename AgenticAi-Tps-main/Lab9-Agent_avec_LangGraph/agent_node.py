...
if __name__ == "__main__":
    result = agent.invoke({"messages": [HumanMessage(content="Add 15 and 27.")], "llm_calls": 0})
    print(result)

    print("\n--- Historique conversationnel (Add 15 and 27) ---")
    ...
    print("\n--- Stream 'updates' (Multiply 12 and 8) ---")
    for chunk in agent.stream(
        {"messages": [HumanMessage(content="Multiply 12 and 8.")], "llm_calls": 0},
        stream_mode="updates",
    ):
        print(chunk)

    # Stream messages (LLM tokens + métadonnées)
    print("\n--- Stream 'messages' (Divide 100 and 4) ---")
    for message_chunk, metadata in agent.stream(
        {"messages": [HumanMessage(content="Divide 100 and 4.")], "llm_calls": 0},
