...
result = agent.invoke({"messages": [HumanMessage(content="Add 7 and 19.")], "llm_calls": 0}, config=config)
...
result = agent.invoke({"messages": [HumanMessage(content="Multiply 6 and 13.")], "llm_calls": 0}, config=config_reject)
...
new_config = agent.update_state(
    picked.config,
    values={"messages": [HumanMessage(content="Multiply 6 and 13.")], "llm_calls": 0},
)
